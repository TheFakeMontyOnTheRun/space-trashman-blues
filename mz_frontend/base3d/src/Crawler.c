...
const char *Crawler_options[3] = {
        "Play game", "Credits", "Help"};

const enum EGameMenuState Crawler_nextStateNavigation[3] = {
        kResumeCurrentState, kResumeCurrentState, kResumeCurrentState};

const int kCrawlerOptionsCount = 3;
...
uint32_t crawlerFrame = 0;
void Crawler_repaintCallback(void) {
    char buffer[32];
    sprintf(&buffer[0], "%d", crawlerFrame++ );
    uint8_t optionsHeight = 8 * kCrawlerOptionsCount;
