...
NSString *fileNamesForSounds[] = {@"menu_move", @"menu_select", @"gotclue", @"detected2", @"bong", @"fire",
								  @"enemyfire", @"derelicttheme"};
...
- (void)drawRect:(NSRect)rect {
	...
	ref = CGImageCreate(320, 200, 8, 32, 4 * 320, rgb, kCGBitmapByteOrder32Host | kCGImageAlphaNoneSkipLast, provider,
						NULL, 0, kCGRenderingIntentDefault);
	CGContextScaleCTM(context, multiplier, yMultiplier);
	CGContextDrawImage(context, CGRectMake(((bounds.size.width / multiplier) - 320) / 2,
										   ((bounds.size.height / yMultiplier) - 200) / 2, 320, 200), ref);
	CGImageRelease(ref);
